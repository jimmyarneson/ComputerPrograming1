...
class MainForm(Form):
	...
	def InitializeComponent(self):
		...
		self._timerball.Tick += self.TimeballTick
		# 
		# timermulti
		# 
	...
	def TimeballTick(self, sender, e):
		ball = self._lblball
		lpdl = self._lblleft
		rpdl = self._lblright
		...
		if self._timerboolean.Enabled == True:
			if self.Width // 2 <= ball.Left:
				return
			if self.Width // 2 > ball.Left:
				if (ball.Left - lpdl.Right) < 0:
					Speed = 0
				elif (ball.Left - lpdl.Right) < (self.Width // 4):
					Speed = 3.5
				elif (ball.Left - lpdl.Right) < (self.Width // 3):
					Speed = 3
				elif (ball.Left - lpdl.Right) < (self.Width // 2):
					Speed = 2.5
				if lpdl.Top != ball.Top - 20:
					if lpdl.Top < ball.Top - 20:
							lpdl.Top += Speed
					if lpdl.Top > ball.Top - 20:
 						lpdl.Top -= Speed
	...
	def MainFormKeyDown(self, sender, e):
		...
		if e.KeyCode == Keys.Z:
			...
			tmult.Enabled = False
			tball.Enabled = False
			self._lbltitle.Text = "Secret Menu-Press B to make ball bigger-Press Space to Change colors to pink and yellow-Backspace take away a point from opponent"
		...
		if tmult.Enabled and tball.Enabled:
			"""forboth keys, also need to set tboolenabled = false"""
			if e.KeyCode == Keys.W:
				self.flagleft = False
				tleft.Enabled = True
	...
	def MainFormLoad(self, sender, e):
		"""TODO : ADD 3 UNIQUE SECERETS/CHEATS/EASTER EGGS IN TOTAL AND FINISH MULTIPLAYER AND SCOREBOARD DUMMY AI"""
		self.balld = 1
		self.ballup = self.R.Next(-4, 5)
		self._timerball.Enabled = False
